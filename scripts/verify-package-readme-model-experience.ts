...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/bundle/enterprise': { kind: 'none', reason: 'The enterprise patch carrier stacks trust Host enforcement; no row registers model-visible request context.' },
  ...
  'packages/bundle/trust-audit-cli': { kind: 'none', reason: 'The export app runs no model turn: it parses one invocation, reads the audit provider, and writes the payload to stdout or a file.' },
  ...
  'packages/credentials/credentials-vault': { kind: 'indirect', reason: 'The Vault provider resolves credential references; consumers of ctx.credentials own any model-facing behavior.' },
  ...
  'packages/trust/trust-identity': { kind: 'none', reason: 'Host ingress identity seam; registers no model-visible request context.' },
  'packages/trust/trust-identity-oidc': { kind: 'none', reason: 'Identity provider backend; authentication stays outside model request assembly.' },
  'packages/trust/trust-authorization': { kind: 'none', reason: 'RBAC evaluation runs before tool execution; registers no model-visible request context.' },
  'packages/trust/trust-authorization-rbac': { kind: 'none', reason: 'Config-driven RBAC guard; enforcement stays outside model request assembly.' },
  'packages/trust/trust-log': { kind: 'none', reason: 'Session integrity metadata sidecar; verification does not alter model-visible events.' },
  'packages/trust/trust-log-jsonl': { kind: 'none', reason: 'Hash-chain persistence provider; integrity metadata stays outside model requests.' },
  'packages/trust/trust-audit': { kind: 'none', reason: 'Parallel enterprise audit vocabulary; records never enter model requests.' },
  'packages/trust/trust-audit-file': { kind: 'none', reason: 'Append-only audit sink; export stays outside model request assembly.' },
  'packages/trust/trust-audit-otel': { kind: 'none', reason: 'OTLP audit export sink; telemetry stays outside model requests.' },
  'packages/trust/trust-lease': { kind: 'none', reason: 'Capability lease registry; fencing tokens gate Host execution only.' },
  'packages/trust/trust-lease-runner': { kind: 'none', reason: 'Dynamic plugin lease checks; enforcement stays outside model requests.' },
  'packages/trust/trust-admission': { kind: 'none', reason: 'Signed composition admission; validation runs at app-boot only.' },
  'packages/trust/trust-admission-boot': { kind: 'none', reason: 'App-boot admission hook; validates composition before Host surfaces mount.' },
  'packages/trust/trust-gateway': { kind: 'none', reason: 'Host RPC identity guard; rejects unauthenticated calls before business methods run.' },
  'packages/trust/trust-approval-audit': { kind: 'none', reason: 'Mirrors approval decisions to trust audit; audit records stay outside model requests.' },
  'packages/trust/trust-approval-grants': { kind: 'none', reason: 'Admin-scoped persistent approval grants; policy stays outside model request assembly.' },
}
