...
Hash-chain sidecar for JSONL session persistence. Mount it in an enterprise composition when Host surfaces need JSONL sidecar provider. Default `web` and `headless` profiles omit this package.
...
Choose it when enterprise Host enforcement needs JSONL sidecar provider. Omit it from developer-preview profiles that stay anonymous.
...
The package registers ctx.trustLog. Source lives under `src/`.
...
- **Phase 1 scope** — Sidecar format is pre-release and rejects foreign files.
