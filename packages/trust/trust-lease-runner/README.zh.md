...
Lease checks for cordis-host-runner dynamic plugins。当 Host 面需要dynamic plugin fencing时，在企业 composition 中挂载它。默认 `web` 与 `headless` profile 不包含此包。
...
当企业 Host 强制需要dynamic plugin fencing时选用。匿名开发者预览 profile 应省略。
...
该包consumes ctx.trustLease。源码位于 `src/`。
...
- **Phase 1 范围** — Only cordis-host-runner paths are covered in Phase 1.
