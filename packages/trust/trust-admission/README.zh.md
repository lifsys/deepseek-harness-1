...
Signed composition admission for enterprise bundles。当 Host 面需要signed bundle admission时，在企业 composition 中挂载它。默认 `web` 与 `headless` profile 不包含此包。
...
当企业 Host 强制需要signed bundle admission时选用。匿名开发者预览 profile 应省略。
...
- **Phase 1 范围** — Signature verification uses config digest validation only in Phase 1.
