...
Config-driven RBAC provider and tools pre-execute guard。当 Host 面需要config-driven roles时，在企业 composition 中挂载它。默认 `web` 与 `headless` profile 不包含此包。
...
当企业 Host 强制需要config-driven roles时选用。匿名开发者预览 profile 应省略。
...
该包registers ctx.trustAuthorization。源码位于 `src/`。
...
- [包组地图](../README.zh.md)
- [子系统参考](../../../docs/subsystems/trust-authorization.zh.md)
...
- **Phase 1 范围** — Resource-level ABAC remains future work.
