...
Service Definition for tamper-evident session log integrity metadata。当 Host 面需要hash-chain verification时，在企业 composition 中挂载它。默认 `web` 与 `headless` profile 不包含此包。
...
当企业 Host 强制需要hash-chain verification时选用。匿名开发者预览 profile 应省略。
...
- **Phase 1 范围** — Signed Ed25519 checkpoints remain optional hardening.
