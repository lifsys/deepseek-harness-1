...
OTLP audit export provider。当 Host 面需要OTLP export sink时，在企业 composition 中挂载它。默认 `web` 与 `headless` profile 不包含此包。
...
当企业 Host 强制需要OTLP export sink时选用。匿名开发者预览 profile 应省略。
...
该包composes with ctx.trustAudit。源码位于 `src/`。
...
- **Phase 1 范围** — Requires outbound network to the collector endpoint.
