...
Service Definition for RBAC evaluation and resource scopes。当 Host 面需要RBAC only时，在企业 composition 中挂载它。默认 `web` 与 `headless` profile 不包含此包。
...
当企业 Host 强制需要RBAC only时选用。匿名开发者预览 profile 应省略。
...
该包ctx.trustAuthorization。源码位于 `src/`。
...
- **Phase 1 范围** — Attribute-based policies remain future work.
