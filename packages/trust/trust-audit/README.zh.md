...
Service Definition for enterprise audit records and export。当 Host 面需要parallel audit vocabulary时，在企业 composition 中挂载它。默认 `web` 与 `headless` profile 不包含此包。
...
当企业 Host 强制需要parallel audit vocabulary时选用。匿名开发者预览 profile 应省略。
...
- **Phase 1 范围** — CEF export remains planned.
